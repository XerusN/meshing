        ...
        2 => {
            ...
            for i in 0..resolution {
                for j in 0..resolution {
                    vertices.push(build_coordinates((i as f64 * l_x) / (resolution as f64 - 1.0), (j as f64 * l_y) / (resolution as f64 - 1.0)))
                }
            }
        },
        _ => {
            let resolution = 100;
            let (l_x, l_y) = (2.0, 1.0);
            let circle_center = build_coordinates(1.0, 0.0);
            let circle_radius = 0.25;
            for i in 0..resolution {
                for j in 0..resolution {
                    let (x, y) = ((i as f64 * l_x) / (resolution as f64 - 1.0), (j as f64 * l_y) / (resolution as f64 - 1.0));
                    if (x - circle_center.x) * (x - circle_center.x) + (y - circle_center.y) * (y - circle_center.y) >= circle_radius * circle_radius {
                        vertices.push(build_coordinates(x, y));
                    }
                }
            }
        },
